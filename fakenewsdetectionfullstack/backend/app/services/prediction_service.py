...
def predict_production(text: str) -> ProductionResponse:
    ...
    logger.info("Running RoBERTa\nPrediction: %s | Confidence: %.1f%%\n", roberta_res["prediction"], roberta_res["confidence"])
    logger.info("Inference Time: %.1f ms\n=====================================\nPrediction Completed\n=====================================", elapsed_ms)
...
def predict_research(text: str) -> ResearchResponse:
    ...
    logger.info("  > BERT       | Prediction: %-4s | Confidence: %5.1f%%", bert_res["prediction"], bert_res["confidence"])
    
    distilbert_res = predict_distilbert(text)
    logger.info("  > DistilBERT | Prediction: %-4s | Confidence: %5.1f%%", distilbert_res["prediction"], distilbert_res["confidence"])
    
    roberta_res = predict_roberta(text)
    logger.info("  > RoBERTa    | Prediction: %-4s | Confidence: %5.1f%%", roberta_res["prediction"], roberta_res["confidence"])
    
    xgboost_res = predict_xgboost(text)
    logger.info("  > XGBoost    | Prediction: %-4s | Confidence: %5.1f%%", xgboost_res["prediction"], xgboost_res["confidence"])
    
    elapsed_ms = round((time.perf_counter() - start_time) * 1000.0, 1)

    voting_res = _majority_vote(bert_res, distilbert_res, roberta_res, xgboost_res)
    
    logger.info("\n  > Majority Voting | Final Prediction: %s | Confidence: %.1f%%", voting_res["final_prediction"], voting_res["confidence"])
    ...
    response = ResearchResponse(
        mode="research",
        bert=ModelPrediction(prediction=bert_res["prediction"], confidence=bert_res["confidence"]),
        distilbert=ModelPrediction(prediction=distilbert_res["prediction"], confidence=distilbert_res["confidence"]),
        roberta=ModelPrediction(prediction=roberta_res["prediction"], confidence=roberta_res["confidence"]),
        xgboost=ModelPrediction(prediction=xgboost_res["prediction"], confidence=xgboost_res["confidence"]),
